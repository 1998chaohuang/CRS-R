        ...
        if (objList != null && objList.size() > 0) {
            ...
            try {
                ...
                for (int j = 0; j < objList.size(); j++) {
                    ExcelData projectBean = (ExcelData) objList.get(j);
                    List<String> list = new ArrayList<>();

                    list.add(projectBean.name);
                    list.add(projectBean.roomNum);
                    list.add(projectBean.num);
                    list.add(projectBean.result);
                    list.add(projectBean.reason);
                    list.add(projectBean.fabingDate);
                    list.add(projectBean.ruyuanDate);
                    list.add(projectBean.writeDate);
                    list.add(projectBean.score);
                ...
                writeBook.write();
                workbook.close();
                Toast.makeText(context, "导出Excel成功"+fileName, Toast.LENGTH_SHORT).show();

////
//                File mfile = new File(f);
//                File[] files = mfile.listFiles();
//                for(int i =0;i< files.length;i++)
//                {Log.v("debug",files[i].getName());
//                }
////
